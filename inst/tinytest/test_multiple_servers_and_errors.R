...
if (FALSE) { # Disabled to prevent segfaults
    # Test 1: Multiple servers can run simultaneously
    cat("Testing multiple servers...\n")

    # Start multiple servers on different ports
    h1 <- runServer(dir = getwd(), addr = "127.0.0.1:8201", blocking = FALSE, silent = TRUE)
    h2 <- runServer(dir = getwd(), addr = "127.0.0.1:8202", blocking = FALSE, silent = TRUE)
    h3 <- runServer(dir = getwd(), addr = "127.0.0.1:8203", blocking = FALSE, silent = TRUE)
    ...
    servers <- listServers()
    expect_true(length(servers) >= 3, info = "Should have at least 3 servers running")
    ...
    expect_true(length(servers_after_shutdown) == length(servers) - 1,
    ...
    # Start a server
    h_main <- runServer(dir = getwd(), addr = "127.0.0.1:8210", blocking = FALSE, silent = TRUE)
    Sys.sleep(0.5)
    ...
    h_conflict <- runServer(dir = getwd(), addr = "127.0.0.1:8210", blocking = FALSE, silent = TRUE)
    ...
    expect_true(length(servers) >= 1, info = "Original server should still be running")
    ...
    expect_error(
        runServer(dir = "/nonexistent/directory", addr = "127.0.0.1:8211", blocking = FALSE),
    ...
    local({
        h_temp1 <- runServer(dir = getwd(), addr = "127.0.0.1:8220", blocking = FALSE, silent = TRUE)
        h_temp2 <- runServer(dir = getwd(), addr = "127.0.0.1:8221", blocking = FALSE, silent = TRUE)
        Sys.sleep(0.5)
    ...
    tryCatch(
        {
            ...
            expect_true(length(servers_controlled) >= length(ports),
    ...
    expect_true(length(servers_config) >= 3,
