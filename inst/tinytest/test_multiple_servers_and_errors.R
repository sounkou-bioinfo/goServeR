...
# Test multiple servers and error handling
# Previously disabled due to segfault in listServers() - now fixed with snapshot pattern

# Test 1: Multiple servers can run simultaneously
cat("Testing multiple servers...\n")

# Start multiple servers on different ports with delays
h1 <- runServer(
  dir = getwd(),
  addr = "127.0.0.1:8201",
  blocking = FALSE,
...
for (i in seq_along(ports)) {
  ...
  Sys.sleep(0.5) # Delay between starts
}
...
for (h in handles) {
  shutdownServer(h)
  Sys.sleep(0.3) # Delay between stops
}
