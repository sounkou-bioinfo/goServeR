...
expect_true(
  ...
  info = "Server handle should be external pointer"
)
Sys.sleep(0.5)
...
shutdownServer(h1)
Sys.sleep(0.5)
cat("   Done: Basic lifecycle completed\n")
...
for (i in seq_along(ports)) {
  ...
  Sys.sleep(0.5) # Wait between starts
}
...
expect_true(
  ...
  info = "All sequential servers should be running"
)
...
cat("   Done: Sequential multiple servers completed\n")
...
cat("   Done: Configuration options completed\n")
...
servers_conflict <- listServers()
expect_true(
  length(servers_conflict) >= 1,
...
cat("   Done: Controlled error scenarios completed\n")
...
expect_error(
  runServer(dir = "", blocking = FALSE),
  info = "Empty directory should error"
)

expect_error(
  runServer(dir = "/nonexistent/path", blocking = FALSE),
  info = "Non-existent directory should error"
)

expect_error(
  runServer(dir = getwd(), addr = "", blocking = FALSE),
  info = "Empty address should error"
)

expect_error(
  runServer(dir = getwd(), addr = "invalid-format", blocking = FALSE),
...
# Type validation
expect_error(
  runServer(dir = getwd(), blocking = "yes"),
  info = "Non-logical blocking should error"
)

expect_error(
  runServer(dir = getwd(), cors = "true"),
  info = "Non-logical cors should error"
)

cat("   Done: R-level validation completed\n")
