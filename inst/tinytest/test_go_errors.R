...
if (interactive() || !nzchar(Sys.getenv("CI"))) {
    ...
    cat("Starting conflicting server (should fail at Go level and be logged)...\n")
    ...
    expect_true(is.list(servers_after), info = "R should still be responsive after Go error")
    ...
    if (.Platform$OS.type != "windows") { # Skip on Windows
        cat("Attempting to bind to privileged port (should fail and be logged)...\n")
        ...
        expect_true(is.list(servers_priv), info = "R should remain responsive after permission error")
    ...
    expect_true(is.list(servers_tls), info = "R should remain responsive after TLS error")
    ...
    expect_true(is.list(servers_final), info = "R should remain fully functional after stress test")

    # Test that R can still perform complex operations
    test_data <- data.frame(x = 1:10, y = rnorm(10))
    expect_true(nrow(test_data) == 10, info = "R should still perform data operations")
