...
if (interactive() || !nzchar(Sys.getenv("CI"))) {
    ...
    file_logger <- function(handler, message, user) {
        cat(paste0("[FILE] ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), " ", message),
            file = logfile, append = TRUE
        )
    }

    h1 <- runServer(
        dir = getwd(), addr = "127.0.0.1:8400", blocking = FALSE,
    ...
    h2 <- runServer(
        dir = getwd(), addr = "127.0.0.1:8401", blocking = FALSE,
        silent = FALSE, log_handler = console_logger
    )

    Sys.sleep(1)

    expect_true(nchar(captured_output) > 0, info = "Custom console logger should capture output")
    expect_true(grepl("\\[CUSTOM\\]", captured_output), info = "Custom prefix should appear")
    ...
    # Default logger
    h3 <- runServer(dir = getwd(), addr = "127.0.0.1:8402", blocking = FALSE, silent = FALSE)
    ...
    h4 <- runServer(
        dir = getwd(), addr = "127.0.0.1:8403", blocking = FALSE,
        silent = FALSE, log_handler = custom_silent
    )

    # Built-in silent mode
    h5 <- runServer(dir = getwd(), addr = "127.0.0.1:8404", blocking = FALSE, silent = TRUE)
    ...
    h6 <- runServer(
        dir = getwd(), addr = "127.0.0.1:8405", blocking = FALSE,
        silent = FALSE, log_handler = user_data_logger
    )
    ...
    expect_true(length(final_servers) == 0, info = "All servers should be cleaned up")
