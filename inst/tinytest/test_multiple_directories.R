...
if (interactive() || !nzchar(Sys.getenv("CI"))) {
    ...
    test_dirs <- c("test_multidir_data", "test_multidir_docs", "test_multidir_files")
    test_files <- list(
    ...
    expect_true(inherits(h1, "externalptr"), info = "Single dir server handle should be external pointer")
    Sys.sleep(0.5)

    servers <- tryCatch(listServers(), error = function(e) list())
    expect_true(length(servers) >= 1, info = "Single dir server should be running")
    ...
    expect_true(inherits(h2, "externalptr"), info = "Multi dir server handle should be external pointer")
    Sys.sleep(0.5)
    ...
    if (length(servers) > 0) {
        ...
        expect_true(grepl(",", server_info$directory), info = "Server info should show multiple directories")
        expect_true(grepl(",", server_info$prefix), info = "Server info should show multiple prefixes")
    }
    ...
    expect_true(inherits(h3, "externalptr"), info = "Mixed paths server handle should be external pointer")
    Sys.sleep(0.5)
    ...
    expect_true(inherits(h5, "externalptr"), info = "Single element vector should work")
    Sys.sleep(0.5)
    shutdownServer(h5)
    Sys.sleep(0.5)
    ...
    expect_true(inherits(h6, "externalptr"), info = "Root prefix should work in multi-dir setup")
    Sys.sleep(0.5)
    shutdownServer(h6)
    Sys.sleep(0.5)
    ...
    expect_true(inherits(h7, "externalptr"), info = "Multi dir with CORS/COOP should work")
    Sys.sleep(0.5)
    shutdownServer(h7)
    Sys.sleep(0.5)
    ...
    servers <- tryCatch(listServers(), error = function(e) list())
    expect_true(length(servers) >= 2, info = "Should have multiple concurrent servers")
