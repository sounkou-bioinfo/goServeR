...
if (interactive() || !nzchar(Sys.getenv("CI"))) {
    ...
    end_time <- Sys.time()
    cat("Server creation took:", as.numeric(end_time - start_time, units = "secs"), "seconds\n")

    # Check servers immediately - should work with proper synchronization
    servers <- listServers()
    expect_true(length(servers) >= length(ports),
        info = paste("Should have at least", length(ports), "servers, got", length(servers))
    )
    ...
    end_time <- Sys.time()
    cat("Server shutdown took:", as.numeric(end_time - start_time, units = "secs"), "seconds\n")
    ...
    cat("Server counts during shutdown:", length(servers1), "->", length(servers2), "->", length(servers3), "->", length(servers4), "\n")
